...
def inspect_repository(path: str | Path, *, include_docs: bool = True) -> RepositoryModel:
    ...
    files = tuple(
        parse_file(root, file) for file in discover_files(root, include_docs=include_docs)
    )
    return RepositoryModel(root=root, files=files)


def pack_repository(
    path: str | Path, *, task: str, max_tokens: int, include_docs: bool = True
) -> ContextPackage:
    ...
    model = inspect_repository(path, include_docs=include_docs)
